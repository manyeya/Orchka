---
title: Node Types
description: Reference for all available node types in Orchka
---

# Node Types

Orchka provides a variety of node types organized into categories. Each node serves a specific purpose in your workflow.

## Triggers

Triggers are the starting points of your workflows. They define when and how a workflow begins execution.

### Manual Trigger

The default trigger that allows manual execution of workflows.

```yaml
Type: MANUAL_TRIGGER
Category: Triggers
```

**Use Cases:**
- Testing workflows during development
- On-demand automation tasks
- Debugging and troubleshooting

### Cron Trigger

Starts a workflow on a cron schedule.

```yaml
Type: CRON_TRIGGER
Category: Triggers
```

**Configuration:**
- **Name**: Node identifier
- **Cron Pattern**: Schedule expression

---

## Actions

Action nodes perform operations and can transform data.

### HTTP Request

Makes HTTP requests to external APIs and services.

```yaml
Type: HTTP_REQUEST
Category: Actions
```

**Configuration:**

| Field | Description |
|-------|-------------|
| URL | Target endpoint |
| Method | GET, POST, PUT, DELETE, PATCH, OPTIONS, HEAD |
| Headers | Request headers (with enable/disable toggle) |
| Body | Request body (JSON, text, form-data, x-www-form-urlencoded) |
| Auth | None, Bearer, Basic, API Key, or Credential |

**Authentication Options:**

| Type | Description |
|------|-------------|
| None | No authentication |
| Bearer | `Authorization: Bearer <token>` |
| Basic | `Authorization: Basic <base64>` |
| API Key | Custom header with API key |
| Credential | Use a stored credential |

**Advanced Settings:**
- **Timeout**: Request timeout in milliseconds (default: 30000)
- **Follow Redirects**: Automatically follow HTTP redirects
- **Retry on Failure**: Retry failed requests with exponential backoff
- **Max Retries**: Maximum retry attempts (default: 3)

### Social Media Actions

Publish to popular social platforms with dedicated nodes and credential types.

| Node | Type | Credential |
|------|------|------------|
| X Post | `X_POST` | X |
| LinkedIn Post | `LINKEDIN_POST` | LinkedIn |
| Facebook Page Post | `FACEBOOK_POST` | Facebook Page |
| Instagram Post | `INSTAGRAM_POST` | Instagram |
| Threads Post | `THREADS_POST` | Threads |
| Discord Message | `DISCORD_MESSAGE` | Discord |
| Reddit Post | `REDDIT_POST` | Reddit |
| Bluesky Post | `BLUESKY_POST` | Bluesky |
| Mastodon Post | `MASTODON_POST` | Mastodon |
| Pinterest Pin | `PINTEREST_PIN` | Pinterest |

See the [Social Media Nodes](/docs/nodes/social-media) reference for platform-specific fields and notes.

---

## AI

AI nodes leverage large language models for intelligent automation.

### AI Agent

Executes AI-powered tasks with tool calling capabilities.

```yaml
Type: AI_AGENT
Category: AI
```

**Configuration:**
- **Model**: AI model to use
- **Credential**: Stored credential for the AI provider (optional)
- **System Prompt**: Instructions for the AI agent
- **Tools**: Connected AI Tool nodes for function calling

**Supported Providers:**
- OpenAI (GPT-4.1, GPT-4o, o1, o3)
- Anthropic (Claude Sonnet 4, Claude 3.7, Claude 3.5 Haiku)
- Google (Gemini 2.5 Pro, Gemini 2.5 Flash, Gemini 2.0 Flash)

**Authentication:**
You can authenticate with AI providers in two ways:
1. **Stored Credentials** (recommended): Select a credential from the dropdown
2. **Environment Variables**: Set `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, etc.

### AI Generate

Simple text generation using AI models.

```yaml
Type: AI_GENERATE
Category: AI
```

**Configuration:**
- **Model**: AI model to use
- **Credential**: Stored credential for the AI provider
- **System Prompt**: Instructions for generation
- **Prompt/Input Variable**: Text to process
- **Temperature**: Creativity level (0-2)
- **Max Tokens**: Maximum output length

**Use Cases:**
- Content creation
- Summarization
- Translation
- Text transformation

### AI Classify

Classify content into predefined categories.

```yaml
Type: AI_CLASSIFY
Category: AI
```

**Configuration:**
- **Model**: AI model to use
- **Categories**: Classification options with labels and descriptions
- **Include Confidence**: Add confidence score (0-1)
- **Include Reasoning**: Add explanation for classification

**Built-in Templates:**
- Sentiment Analysis (positive/negative/neutral)
- Intent Detection (question/request/complaint/feedback)
- Priority Classification (urgent/high/medium/low)
- Spam Detection (spam/not_spam)

## Control

Control nodes manage the flow of execution.

### Condition (If)

Branches workflow based on an expression.

```yaml
Type: IF_CONDITION
Category: Control
```

**Configuration:**
- **Expression**: JSONata expression that evaluates to boolean
- **True Branch**: Path when expression is true
- **False Branch**: Path when expression is false

**Example Expressions:**
```
$input.status = "success"
$input.count > 10
$exists($input.email)
```

### Wait

Pauses workflow execution for a specified duration or until a specific time.

```yaml
Type: WAIT
Category: Control
```

**Configuration:**

| Mode | Fields | Description |
|------|--------|-------------|
| Duration | `value`, `unit` | Wait for a fixed time period |
| Until | `timestamp` | Wait until a specific date/time |

**Duration Units:** seconds, minutes, hours, days

**Example - Duration Mode:**
```json
{
  "mode": "duration",
  "duration": {
    "value": 5,
    "unit": "minutes"
  }
}
```

**Example - Until Mode:**
```json
{
  "mode": "until",
  "until": "{{ $input.scheduledTime }}"
}
```

### Loop

Iterates over an array of items or a fixed count.

```yaml
Type: LOOP
Category: Control
```

**Configuration:**

| Mode | Fields | Description |
|------|--------|-------------|
| Array | `arrayExpression` | Iterate over each element |
| Count | `count` | Iterate N times |

**Loop Context Variables:**
- `$item` - Current item in the iteration
- `$index` - Current index (0-based)
- `$total` - Total number of items

**Example - Array Mode:**
```json
{
  "mode": "array",
  "arrayExpression": "{{ $input.users }}"
}
```

**Example - Count Mode:**
```json
{
  "mode": "count",
  "count": 10
}
```

### Switch

Routes workflow execution based on matching a value against multiple cases.

```yaml
Type: SWITCH
Category: Control
```

**Configuration:**
- **Expression**: Value to evaluate and match
- **Cases**: Array of value/label pairs to match against
- **Default**: Fallback when no case matches

**Example Configuration:**
```json
{
  "expression": "{{ $input.status }}",
  "cases": [
    { "id": "case-1", "value": "pending", "label": "Pending" },
    { "id": "case-2", "value": "approved", "label": "Approved" },
    { "id": "case-3", "value": "rejected", "label": "Rejected" }
  ]
}
```

**Output Handles:**
- One handle per case (connects to case-specific logic)
- Default handle (when no case matches)

### Merge

Converges multiple incoming branches into one output.

```yaml
Type: MERGE
Category: Control
```

**Configuration:**

| Mode | Description |
|------|-------------|
| Append | Flatten all inputs into one array |
| Merge by Key | Join rows using a shared key field |
| Keep First | Use the first non-empty value |
| Keep Last | Use the last non-empty value |
| Combine | Merge object fields |
| Custom Expression | Use a JSONata expression |

See the [Merge Node](/docs/nodes/merge) reference for examples and custom expression inputs.

---

## Next Steps

<Cards>
  <Card title="Expressions" href="/docs/expressions">
    Learn to transform data between nodes.
  </Card>
  <Card title="AI Agent" href="/docs/nodes/ai-agent">
    Master AI-powered automation with tools.
  </Card>
  <Card title="AI Generate" href="/docs/nodes/ai-generate">
    Generate text content with AI.
  </Card>
  <Card title="AI Classify" href="/docs/nodes/ai-classify">
    Classify content into categories.
  </Card>
  <Card title="Social Media Nodes" href="/docs/nodes/social-media">
    Publish to social platforms.
  </Card>
  <Card title="Merge Node" href="/docs/nodes/merge">
    Combine multiple workflow branches.
  </Card>
</Cards>
