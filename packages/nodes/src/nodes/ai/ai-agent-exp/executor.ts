 ...
 * Uses AI SDK generateText with tools for agent execution
 */
...
export const aiAgentExpExecutor: NodeExecutor<AIAgentExpSettings> = async ({
  ...
  await publishNodeStatus(publish, nodeId, "loading", NodeType.AI_AGENT_EXP);
  ...
  try {
    ...
    // Create model and execute AI agent
    ...
    if (data.enabledTools.includes("calculator")) {
      tools.calculator = tool({
        ...
        inputSchema: z.object({
          operation: z.enum(["add", "subtract", "multiply", "divide", "power", "sqrt", "percentage"]),
        ...
        execute: async ({ operation, a, b }) => {
          ...
          switch (operation) {
            ...
            case "divide": if (b === 0) return "Error: Division by zero"; result = a / (b ?? 1); break;
            case "power": result = Math.pow(a, b ?? 2); break;
            case "sqrt": result = Math.sqrt(a); break;
            case "percentage": result = (a / 100) * (b ?? 100); break;
    ...
    if (data.outputFormat === "json" && typeof result.text === "string") {
      try { output = JSON.parse(result.text); } catch { output = result.text; }
    }

    await publishNodeStatus(publish, nodeId, "success", NodeType.AI_AGENT_EXP);
    ...
    await publishNodeStatus(publish, nodeId, "error", NodeType.AI_AGENT_EXP);
    throw error;
  }
};
