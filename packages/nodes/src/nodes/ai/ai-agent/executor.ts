...
import { CredentialType, isOpenAICredential, isAnthropicCredential, isGoogleAICredential } from "@orchka/credentials-core";
...
export const aiAgentExecutor: NodeExecutor<AIAgentSettings> = async ({
  ...
  try {
    ...
    await publishNodeStatus(publish, nodeId, "error", NodeType.AI_AGENT);
    throw error;
  }
};
...
function extractAICredentialConfig(
  ...
  throw new NonRetriableError(`Unsupported credential type for AI agent: ${type}`);
}
...
function createModel(modelString: string, credentialConfig: AICredentialConfig | null): BaseChatModel {
  ...
  if (modelString.startsWith("claude-")) {
    return new ChatAnthropic({
      model: modelString,
      temperature: 0.7,
      ...(credentialConfig && { apiKey: credentialConfig.apiKey }),
    });
  }
    ...
    for (const customTool of data.customTools) {
      try {
        tools.push(new DynamicStructuredTool({
          ...
          schema: z.object({ input: z.string().describe("Input for the custom tool") }),
          func: async ({ input }) => {
      ...
      } catch { /* Skip invalid custom tools */ }
    }
  }
